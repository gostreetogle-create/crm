...
## UX standardization batch #5 (keyboard focus clarity)

Статус: `Done` (safe UI-only update).

- Добавлены `focus-visible` состояния в `crud-row-actions-menu`:
  - trigger-кнопка меню действий;
  - пункты меню (включая `danger` вариант).
- Ожидаемый UX-эффект:
  - лучшее управление с клавиатуры;
  - выше заметность текущего фокуса;
  - ниже риск ошибки при последовательном выборе действий в строке.
