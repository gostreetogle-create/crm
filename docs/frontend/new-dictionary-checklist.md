...
**Поведение полей (таблица / модалка / сценарии / массовый импорт):** [`dictionary-field-behavior-guide.md`](./dictionary-field-behavior-guide.md).
...
8. **Клиенты** — контрагенты и наценка для КП; см. `clients-crud.md`.
...
- [ ] `CrudLayout` на плитке — **без** Excel (массовый импорт только через Admin JSON API и права `admin.bulk.*`, см. админ-настройки и `docs/dev-bulk-json-migration-checklist.md`)
...
- [ ] Визуал сверен с `/demo` (секция «Плитка справочника: варианты (визуальный эталон)»)
- [ ] Нет расхождений Demo vs `/dictionaries` по базовому каркасу таблицы
- [ ] Нет расхождений Demo vs `/dictionaries` по каркасу `CrudLayout` (без скрытого Excel-toolbar; массовый импорт не на плитке)
