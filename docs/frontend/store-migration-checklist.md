...
Критерий done:
- [ ] Нельзя получить расхождение: на одной странице статус = черновик, на другой = согласование — **проверить вручную (smoke)**.
...
Критерий done:
- [ ] Копирование КП из разных мест даёт одинаковый результат — **регрессионная проверка по желанию**.
...
Критерий done:
- [ ] Нет конфликтов прав — **проверить вручную при смене ролей**.
...
Статус: **ручной прогон по чеклисту ниже** (Playwright покрывает только базовый вход в хаб и сценарий КП→заказ; полный интерактив — здесь).
