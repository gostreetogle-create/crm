...
2. Визуальный эталон плитки хаба: на `/demo` секция **«Плитка справочника: варианты (визуальный эталон)»** — сравнение вариантов композиции (хаб с раскрытием, полный список без кнопки, превью без раскрытия, узкая карточка) и та же базовая цепочка `dictionaryGrid` → `dictionaryTile` → при необходимости `app-hub-crud-expandable-shell` → `content-card` → `CrudLayout`, что на `/dictionaries`. Сами плитки витрины рендерятся **вне** внешней `content-card` intro, в том же `dictionaryGrid`, что и хаб, чтобы ширина колонки (≈ одна четверть `.page`) совпадала с `/dictionaries`. Превью **одной строки** и кнопка раскрытия на продуктовом хабе: обёртка `app-hub-crud-expandable-shell` (`shared/ui/hub-crud-expandable/`), состояние по ключу плитки в `HubCrudExpandStateService`, привязка к `CrudLayout` через `maxTableBodyRows` / `tableBodyMaxHeight` (как в `dictionaries-page` и вариант 1 на demo). Раскрытый список рисуется **поверх** контента ниже плитки (без проталкивания страницы); высота резерва в потоке при открытии берётся из **фактической** высоты свёрнутой панели (`ResizeObserver`), иначе — fallback `layoutReserveWhenOpen`. Клик **вне** плитки сворачивает её; кнопка с тремя линиями остаётся внизу раскрытой панели.
