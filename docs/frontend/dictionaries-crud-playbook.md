...
2. Визуальный эталон плитки хаба: на `/demo` карточка **«Эталон плитки справочника (как на /dictionaries)»** — та же оболочка `dictionaryGrid` → `dictionaryTile` → `content-card` → `CrudLayout`, что на `/dictionaries`.
