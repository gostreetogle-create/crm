...
const MOCK_MODEL: MaterialGeometryModel = {
  version: '0.2',
  materialFields: [
    ...
    { key: 'notes', label: 'Заметки', type: 'string', required: false },
    {
      ...
      type: 'boolean',
      required: false,
      comment: 'Деактивация вместо удаления',
    },
  ],
  geometryFields: [
    { key: 'id', label: 'Идентификатор', type: 'uuid', required: true, comment: 'PK' },
    ...
    { key: 'name', label: 'Наименование', type: 'string', required: true },
    {
      ...
      type: 'string',
      required: true,
      comment: 'Ключ типа/формулы: rectangular / cylindrical / tube / plate / custom',
    },
...
@Injectable()
export class MaterialGeometryMockRepository implements MaterialGeometryRepository {
  getModel(): Observable<MaterialGeometryModel> {
    return of(MOCK_MODEL);
  }
}
