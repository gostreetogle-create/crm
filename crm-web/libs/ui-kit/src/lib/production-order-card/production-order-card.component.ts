...
export class ProductionOrderCardComponent {
  private loggedFirstPhotoSrc = false;
  ...
  resolvePhotoUrl(line: ProductionOrderCardLine | undefined): string {
    ...
    const src = raw.startsWith('/') ? (base ? `${base}${raw}` : raw) : (base ? `${base}/media/trade-goods/${raw}` : `/media/trade-goods/${raw}`);
    if (!this.loggedFirstPhotoSrc && line?.lineNo === 1) {
      // Temporary runtime check for photo URL resolution.
      console.log('[production-order-card] first line photo src:', src);
      this.loggedFirstPhotoSrc = true;
    }
    return src;
  }
