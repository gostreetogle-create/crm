...
import { DbBackupListItemDto, DbBackupsAdminService } from '@srm/settings-core';

@Injectable({ providedIn: 'root' })
export class DbBackupsAdminStore {
  ...
  reload(): void {
    ...
    forkJoin({ schedule: this.api.getSchedule(), list: this.api.list() })
      ...
      .subscribe({
        ...
        error: () => this.errorText.set('Не удалось загрузить данные бэкапов (расписание или список архивов).'),
      });
  }
  ...
  saveSchedule(): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err) => {
          if (err?.status === 400) this.errorText.set('Некорректное время (ожидается формат ЧЧ:ММ).');
          else this.errorText.set('Не удалось сохранить расписание.');
        },
      });
  }
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.errorText.set(this.backupCommandErrorMessage(err, 'Не удалось восстановить БД из архива.'));
        },
      });
  }
